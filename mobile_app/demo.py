...
def get_random_cert() :
    ...
    with open('training_courses.txt') as f :
        ...
        while len(cert)<5 :  # sure its not an empty line
            cert = courses[randint(0,len(courses)-1)]
...
def task_generator():
    ...
    category = available_categories[randint(0,len(available_categories)-1)]
...
params={'id':'',
        'events' : resolve_conflicts([event_generator() for i in range(50)]),
        'tasks' : [task_generator() for i in range(100)]}
